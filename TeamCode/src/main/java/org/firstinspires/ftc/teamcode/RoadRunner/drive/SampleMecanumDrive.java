...
public class SampleMecanumDrive extends MecanumDrive {
    ...
    public static PIDCoefficients TRANSLATIONAL_PID = new PIDCoefficients(5, 0.1, 0.02);
    public static PIDCoefficients HEADING_PID = new PIDCoefficients(5, 0.1, 0.02);
    ...
    public double DISTANCE_BETWEEN_SENSORS = 139; //MM
    ...
    public SampleMecanumDrive(HardwareMap hardwareMap) {
        ...
        follower = new HolonomicPIDVAFollower(TRANSLATIONAL_PID, TRANSLATIONAL_PID, HEADING_PID,
                new Pose2d(0.5, 0.5, Math.toRadians(2)), 0.6);
    ...
    public void RobotCentricSlowed(Gamepad g) {
        if(isAligning){return;}
        double y = -g.left_stick_y; // Remember, Y stick is reversed!
        double x = g.left_stick_x*1.1;
        double rx = g.right_stick_x/1.25;
        ...
        leftFront.setPower((y + x + rx) * speed);
        leftRear.setPower((y - x + rx) * speed);
        rightFront.setPower((y - x - rx) * speed);
        rightRear.setPower((y + x - rx) * speed);
    }
    ...
    public void alignToBackdrop(double rd, double ld){
        isAligning = true;
        double alpha = Math.atan((rd * (rd - ld)) / (DISTANCE_BETWEEN_SENSORS * rd));
        try {
